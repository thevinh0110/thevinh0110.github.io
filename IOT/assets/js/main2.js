...
// }
function getOtherDevices(callback) {
...
function renderOtherDevices(otherDevices) {
    ...
    htmls = htmls.join('')
    bodyOtherDevice.innerHTML = htmls
}
...
function removeItem(index) {
  ...
  itemSelected.forEach((el,id) => {
    ...
    if(dataIndex == index) {
      indexx = dataIndex
      el.remove()
      bodyOtherDevice.appendChild(arr[indexx])
      arr[indexx].style.display = '' 
      // mixFloor()
    }
...
function start() {
    getOtherDevices(renderOtherDevices)
